...
if [[ ! -z ${GPU_ARCH_BUILD_NAVI10} ]]; then
    if [ ! -v CMAKE_CFG_GPU_ARCH_DEFAULT ]; then
...
if [[ ! -z ${GPU_ARCH_BUILD_NAVI14} ]]; then
    if [ ! -v CMAKE_CFG_GPU_ARCH_DEFAULT ]; then
...
if [[ ! -z ${GPU_ARCH_BUILD_NAVI21} ]]; then
    if [ ! -v CMAKE_CFG_GPU_ARCH_DEFAULT ]; then
...
if [[ ! -z ${GPU_ARCH_BUILD_REMBRANDT} ]]; then
    if [ ! -v CMAKE_CFG_GPU_ARCH_DEFAULT ]; then
...
if [ ! -v SPACE_SEPARATED_GPU_TARGET_LIST_DEFAULT ]; then
    ...
    echo "    GPU_ARCH_BUILD_VEGA=1"
    echo "    GPU_ARCH_BUILD_NAVI10=1"
    echo "    GPU_ARCH_BUILD_NAVI14=1"
    echo "    GPU_ARCH_BUILD_NAVI21=1"
    echo "    GPU_ARCH_BUILD_REMBRANDT=1"
    exit -1
fi
